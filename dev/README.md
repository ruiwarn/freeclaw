# FreeClaw Development Environment

A fully containerized development sandbox for FreeClaw agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`freeclaw-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `freeclaw` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/freeclaw-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.freeclaw` (mounted to `/freeclaw-data/.freeclaw`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.freeclaw` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `freeclaw-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=freeclaw-target`) to speed repeat local image builds.
