...
/// `FreeClaw` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "freeclaw")]
...
Examples:
  freeclaw agent                              # interactive session
  freeclaw agent -m \"Summarize today's logs\"  # single message
  freeclaw agent -p anthropic --model claude-sonnet-4-20250514
  freeclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  freeclaw gateway                  # use config defaults
  freeclaw gateway -p 8080          # listen on port 8080
  freeclaw gateway --host 0.0.0.0   # bind to all interfaces
  freeclaw gateway -p 0             # random available port")]
    Gateway {
...
Launches the full FreeClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run FreeClaw in \
production or as an always-on assistant.

Use 'freeclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  freeclaw daemon                   # use config defaults
  freeclaw daemon -p 9090           # gateway on port 9090
  freeclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
    ...
    /// - `freeclaw estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `freeclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  freeclaw cron list
  freeclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  freeclaw cron add '*/30 * * * *' 'Check system health'
  freeclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  freeclaw cron add-every 60000 'Ping heartbeat'
  freeclaw cron once 30m 'Run backup in 30 minutes'
  freeclaw cron pause <task-id>
  freeclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect FreeClaw \
...
Examples:
  ...
  freeclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  freeclaw channel remove my-bot
  freeclaw channel bind-telegram freeclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  ...
  freeclaw hardware introspect /dev/ttyACM0
  freeclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: freeclaw::HardwareCommands,
    },
...
Examples:
  ...
  freeclaw peripheral add nucleo-f401re /dev/ttyACM0
  freeclaw peripheral add rpi-gpio native
  freeclaw peripheral flash --port /dev/cu.usbmodem12345
  freeclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: freeclaw::PeripheralCommands,
    },
...
Examples:
  freeclaw memory stats
  freeclaw memory list
  freeclaw memory list --category core --limit 10
  freeclaw memory get <key>
  freeclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  freeclaw config schema              # print JSON Schema to stdout
  freeclaw config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Generate shell completion scripts for `freeclaw`.
...
Examples:
  ...
  freeclaw completions fish > ~/.config/fish/completions/freeclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        std::env::set_var("FREECLAW_CONFIG_DIR", config_dir);
    }
    ...
    } = &cli.command
    {
        ...
        if std::env::var("FREECLAW_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for FreeClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting FreeClaw Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting FreeClaw Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting FreeClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting FreeClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 FreeClaw Status");
            println!();
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `freeclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `freeclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `freeclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `freeclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `freeclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `freeclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["freeclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["freeclaw", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["freeclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["freeclaw", "estop", "resume", "--domain", "*.chase.com"])
