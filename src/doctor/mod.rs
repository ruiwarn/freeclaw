...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 FreeClaw Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `freeclaw doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 FreeClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `freeclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `freeclaw onboard` to set one up",
        ));
    }
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".freeclaw_doctor_probe_")));
    }
