...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("FREECLAW_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("FREECLAW_LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("FREECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
    ...
    println!("🦀 FreeClaw Gateway listening on http://{display_addr}");
...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("freeclaw_heartbeat_ticks_total 1"));
    }
