...
/// and forwards PRIVMSG messages to the `FreeClaw` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :FreeClaw", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn new_stores_all_fields() {
        ...
        assert_eq!(ch.username, "freeclaw");
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("freeclaw"));
