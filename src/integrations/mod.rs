...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `freeclaw onboard --interactive` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: freeclaw onboard --channels-only");
            println!("    4. Start: freeclaw channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: freeclaw onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: freeclaw onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    FreeClaw can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.freeclaw/workspace/cron/");
            println!("    Run: freeclaw cron list");
        }
        "Webhooks" => {
            println!("  Built-in:");
            println!("    HTTP endpoint for external triggers.");
            println!("    Run: freeclaw gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/theonlyhennygod/freeclaw");
            }
        }
    }
