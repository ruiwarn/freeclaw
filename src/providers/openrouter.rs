...
impl Provider for OpenRouterProvider {
    ...
    async fn chat_with_system(
        ...
        let credential = self.credential.as_ref()
            .ok_or_else(|| anyhow::anyhow!("OpenRouter API key not set. Run `freeclaw onboard` or set OPENROUTER_API_KEY env var."))?;
    ...
    async fn chat(
        ...
        let credential = self.credential.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
            "OpenRouter API key not set. Run `freeclaw onboard` or set OPENROUTER_API_KEY env var."
        )
        })?;
