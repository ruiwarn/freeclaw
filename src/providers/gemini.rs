...
//! - FreeClaw auth-profiles OAuth tokens
        ...
        let auth = self.auth.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Gemini API key not found. Options:\n\
                 ...
                 3. Run `freeclaw auth login --provider gemini`\n\
                 4. Get an API key from https://aistudio.google.com/app/apikey\n\
                 5. Run `freeclaw onboard` to configure"
            )
        })?;
        ...
        let (mut oauth_token, mut project) = match auth {
            ...
            GeminiAuth::ManagedOAuth => {
                ...
                let token = auth_service
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(
                            "Gemini auth profile not found. Run `freeclaw auth login --provider gemini`."
                        )
                    })?;
                let proj = self.resolve_oauth_project(&token).await?;
                (Some(token), Some(proj))
            ...
            match auth {
                GeminiAuth::ManagedOAuth => {
                    ...
                    let _token = auth_service
                        ...
                        .ok_or_else(|| {
                            anyhow::anyhow!(
                                "Gemini auth profile not found or expired. Run: freeclaw auth login --provider gemini"
                            )
                        })?;
