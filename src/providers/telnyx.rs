...
///
/// ```rust,ignore
/// use freeclaw::providers::telnyx::TelnyxProvider;
/// use freeclaw::providers::Provider;
///
...
impl TelnyxProvider {
    ...
    /// 2. `FREECLAW_API_KEY` environment variable (fallback)
...
impl Provider for TelnyxProvider {
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `freeclaw onboard`."
            )
        })?;
