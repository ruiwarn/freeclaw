...
const CODEX_RESPONSES_URL_ENV: &str = "FREECLAW_CODEX_RESPONSES_URL";
const CODEX_BASE_URL_ENV: &str = "FREECLAW_CODEX_BASE_URL";
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are FreeClaw, a concise and helpful coding assistant.";
...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        let state_dir = options
            .freeclaw_dir
            .clone()
            .unwrap_or_else(default_freeclaw_dir);
...
fn default_freeclaw_dir() -> PathBuf {
...
fn resolve_reasoning_effort(model_id: &str) -> String {
    let raw = std::env::var("FREECLAW_CODEX_REASONING_EFFORT")
        .ok()
        .and_then(|value| first_nonempty(Some(&value)))
    ...
    async fn send_responses_request(
        ...
        let access_token = if use_gateway_api_key_auth {
            ...
            Some(oauth_access_token.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `freeclaw auth login --provider openai-codex`."
                )
            })?)
        };
        let account_id = if use_gateway_api_key_auth {
            ...
            Some(account_id.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `freeclaw auth login --provider openai-codex` again."
                )
            })?)
        };
