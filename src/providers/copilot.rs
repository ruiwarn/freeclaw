...
/// Tokens are cached to `~/.config/freeclaw/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "freeclaw")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("freeclaw-copilot-{user}"))
            });
