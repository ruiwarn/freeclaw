...
/// Top-level FreeClaw configuration, loaded from `config.toml`.
///
/// Resolution order: `FREECLAW_WORKSPACE` env → `active_workspace.toml` marker → `~/.freeclaw/config.toml`.
...
pub struct Config {
    ...
    /// API key for the selected provider. Overridden by `FREECLAW_API_KEY` or `API_KEY` env vars.
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all FreeClaw-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "freeclaw" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "freeclaw_memories".
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "freeclaw".
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `FREECLAW_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `FREECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
...
/// How FreeClaw receives events from Feishu / Lark.
///
...
fn default_estop_state_file() -> String {
    "~/.freeclaw/estop-state.json".to_string()
}
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to freeclaw dir)
...
impl Default for Config {
    fn default() -> Self {
        ...
        let freeclaw_dir = home.join(".freeclaw");

        Self {
            workspace_dir: freeclaw_dir.join("workspace"),
            config_path: freeclaw_dir.join("config.toml"),
            api_key: None,
            api_url: None,
...
pub(crate) fn resolve_config_dir_for_workspace(workspace_dir: &Path) -> (PathBuf, PathBuf) {
    ...
    let legacy_config_dir = workspace_dir
        .parent()
        .map(|parent| parent.join(".freeclaw"));
...
/// `FREECLAW_CONFIG_DIR` > `FREECLAW_WORKSPACE` > active workspace marker > defaults.
pub(crate) async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    ...
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_freeclaw_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "FREECLAW_CONFIG_DIR",
            Self::EnvWorkspace => "FREECLAW_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_freeclaw_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("FREECLAW_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let freeclaw_dir = PathBuf::from(custom_config_dir);
            return Ok((
                freeclaw_dir.clone(),
                freeclaw_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("FREECLAW_WORKSPACE") {
        if !custom_workspace.is_empty() {
            let (freeclaw_dir, workspace_dir) =
    ...
    if let Some((freeclaw_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_freeclaw_dir).await?
    {
        return Ok((
            freeclaw_dir,
            workspace_dir,
    ...
    Ok((
        default_freeclaw_dir.to_path_buf(),
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'freeclaw'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "FREECLAW_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (freeclaw_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_freeclaw_dir, &default_workspace_dir).await?;

        let config_path = freeclaw_dir.join("config.toml");

        fs::create_dir_all(&freeclaw_dir)
            .await
            .with_context(|| config_dir_creation_error(&freeclaw_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&freeclaw_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        // 1) FREECLAW_PROVIDER always wins when set.
        // 2) FREECLAW_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("FREECLAW_PROVIDER") {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        } else if let Ok(provider) =
            std::env::var("FREECLAW_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
        {
            if !provider.is_empty() {
                self.default_provider = Some(provider);
            }
        ...
        // Model: FREECLAW_MODEL or MODEL
        if let Ok(model) = std::env::var("FREECLAW_MODEL").or_else(|_| std::env::var("MODEL")) {
            if !model.is_empty() {
        ...
        if let Ok(flag) = std::env::var("FREECLAW_OPEN_SKILLS_ENABLED") {
            if !flag.trim().is_empty() {
                match flag.trim().to_ascii_lowercase().as_str() {
                    ...
                    _ => tracing::warn!(
                        "Ignoring invalid FREECLAW_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                    ),
                }
            }
        }

        // Open-skills directory override: FREECLAW_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("FREECLAW_OPEN_SKILLS_DIR") {
        ...
        // Skills prompt mode override: FREECLAW_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("FREECLAW_SKILLS_PROMPT_MODE") {
            if !mode.trim().is_empty() {
                if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                    self.skills.prompt_injection_mode = parsed;
                } else {
                    tracing::warn!(
                        "Ignoring invalid FREECLAW_SKILLS_PROMPT_MODE (valid: full|compact)"
                    );
                }
            }
        }

        // Gateway port: FREECLAW_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("FREECLAW_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: FREECLAW_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("FREECLAW_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
        {
            if !host.is_empty() {
        ...
        // Allow public bind: FREECLAW_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("FREECLAW_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: FREECLAW_TEMPERATURE
        ...
        // Reasoning override: FREECLAW_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("FREECLAW_REASONING_ENABLED")
        ...
        // Web search enabled: FREECLAW_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("FREECLAW_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: FREECLAW_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("FREECLAW_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: FREECLAW_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("FREECLAW_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Web search max results: FREECLAW_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("FREECLAW_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: FREECLAW_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("FREECLAW_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): FREECLAW_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("FREECLAW_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): FREECLAW_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("FREECLAW_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: FREECLAW_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("FREECLAW_STORAGE_CONNECT_TIMEOUT_SECS") {
            if let Ok(timeout_secs) = timeout_secs.parse::<u64>() {
                if timeout_secs > 0 {
        ...
        // Proxy enabled flag: FREECLAW_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("FREECLAW_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: FREECLAW_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("FREECLAW_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("FREECLAW_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("FREECLAW_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("FREECLAW_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("FREECLAW_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid FREECLAW_PROXY_SCOPE (valid: environment|freeclaw|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("FREECLAW_PROXY_SERVICES") {
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let store = crate::security::SecretStore::new(freeclaw_dir, self.secrets.encrypt);
...
mod tests {
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/freeclaw"));
        assert!(msg.contains("/etc/freeclaw"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("freeclaw"));
    }
...
provider = "postgres"
dbURL = "postgres://postgres:postgres@localhost:5432/freeclaw"
schema = "public"
table = "memories"
...
"#;
        ...
        assert_eq!(
            ...
            Some("postgres://postgres:postgres@localhost:5432/freeclaw")
        );
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("freeclaw_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "freeclaw_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("freeclaw_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.freeclaw/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
        };
...
"#;
    ...
    fn clear_proxy_env_test_vars() {
        for key in [
            "FREECLAW_PROXY_ENABLED",
            "FREECLAW_HTTP_PROXY",
            "FREECLAW_HTTPS_PROXY",
            "FREECLAW_ALL_PROXY",
            "FREECLAW_NO_PROXY",
            "FREECLAW_PROXY_SCOPE",
            "FREECLAW_PROXY_SERVICES",
            "HTTP_PROXY",
            "HTTPS_PROXY",
            "ALL_PROXY",
            "NO_PROXY",
    ...
    async fn env_override_api_key() {
        ...
        std::env::set_var("FREECLAW_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("FREECLAW_API_KEY");
    }
    ...
    async fn env_override_provider() {
        ...
        std::env::set_var("FREECLAW_PROVIDER", "anthropic");
        config.apply_env_overrides();
    ...
    async fn env_override_model_provider_alias() {
        ...
        std::env::remove_var("FREECLAW_PROVIDER");
        std::env::set_var("FREECLAW_MODEL_PROVIDER", "openai-codex");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("openai-codex"));

        std::env::remove_var("FREECLAW_MODEL_PROVIDER");
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        std::env::set_var("FREECLAW_OPEN_SKILLS_ENABLED", "true");
        std::env::set_var("FREECLAW_OPEN_SKILLS_DIR", "/tmp/open-skills");
        std::env::set_var("FREECLAW_SKILLS_PROMPT_MODE", "compact");
        config.apply_env_overrides();
        ...
        std::env::remove_var("FREECLAW_OPEN_SKILLS_ENABLED");
        std::env::remove_var("FREECLAW_OPEN_SKILLS_DIR");
        std::env::remove_var("FREECLAW_SKILLS_PROMPT_MODE");
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        std::env::set_var("FREECLAW_OPEN_SKILLS_ENABLED", "maybe");
        std::env::set_var("FREECLAW_SKILLS_PROMPT_MODE", "invalid");
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        std::env::set_var("FREECLAW_PROVIDER", "openrouter");
    ...
    async fn env_override_model() {
        ...
        std::env::set_var("FREECLAW_MODEL", "gpt-4o");
        config.apply_env_overrides();
    ...
    async fn env_override_workspace() {
        ...
        std::env::set_var("FREECLAW_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
        assert_eq!(config.workspace_dir, PathBuf::from("/custom/workspace"));

        std::env::remove_var("FREECLAW_WORKSPACE");
    }
    ...
    async fn resolve_runtime_config_dirs_uses_env_workspace_first() {
        ...
        std::env::set_var("FREECLAW_WORKSPACE", &workspace_dir);
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        std::env::set_var("FREECLAW_CONFIG_DIR", &explicit_config_dir);
        std::env::remove_var("FREECLAW_WORKSPACE");
        ...
        std::env::remove_var("FREECLAW_CONFIG_DIR");
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    async fn load_or_init_workspace_override_uses_workspace_root_for_config() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("freeclaw_test_home_{}", uuid::Uuid::new_v4()));
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let legacy_config_path = temp_home.join(".freeclaw").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".freeclaw");
...
"#,
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        persist_active_workspace_config_dir(&marker_config_dir)
            .await
            .unwrap();
        std::env::set_var("FREECLAW_WORKSPACE", &env_workspace_dir);
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("freeclaw_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".freeclaw");
    ...
    async fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("FREECLAW_PROVIDER", "");
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        std::env::set_var("FREECLAW_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("FREECLAW_GATEWAY_PORT");
    }
    ...
    async fn env_override_gateway_host() {
        ...
        std::env::set_var("FREECLAW_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("FREECLAW_GATEWAY_HOST");
    }
    ...
    async fn env_override_reasoning_enabled() {
        ...
        std::env::set_var("FREECLAW_REASONING_ENABLED", "true");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        std::env::set_var("FREECLAW_REASONING_ENABLED", "maybe");
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_storage_provider_config() {
        ...
        std::env::set_var("FREECLAW_STORAGE_PROVIDER", "postgres");
        std::env::set_var("FREECLAW_STORAGE_DB_URL", "postgres://example/db");
        std::env::set_var("FREECLAW_STORAGE_CONNECT_TIMEOUT_SECS", "15");
        ...
        std::env::remove_var("FREECLAW_STORAGE_PROVIDER");
        std::env::remove_var("FREECLAW_STORAGE_DB_URL");
        std::env::remove_var("FREECLAW_STORAGE_CONNECT_TIMEOUT_SECS");
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        std::env::set_var("FREECLAW_PROXY_ENABLED", "true");
        std::env::set_var("FREECLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var(
            "FREECLAW_PROXY_SERVICES",
            "provider.openai, tool.http_request",
        );
        std::env::set_var("FREECLAW_PROXY_SCOPE", "services");
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        let mut config = Config::default();
        std::env::set_var("FREECLAW_PROXY_ENABLED", "true");
        std::env::set_var("FREECLAW_PROXY_SCOPE", "environment");
        std::env::set_var("FREECLAW_HTTP_PROXY", "http://127.0.0.1:7890");
        std::env::set_var("FREECLAW_HTTPS_PROXY", "http://127.0.0.1:7891");
        std::env::set_var("FREECLAW_NO_PROXY", "localhost,127.0.0.1");
...
[security.estop]
enabled = true
state_file = "~/.freeclaw/estop-state.json"
require_otp_to_resume = true
"#,
        )
        .unwrap();
