...
/// Handle `freeclaw peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: freeclaw peripheral add <board> <path>");
                println!("  Example: freeclaw peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
