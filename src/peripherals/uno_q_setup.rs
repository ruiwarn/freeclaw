//! Deploy FreeClaw Bridge app to Arduino Uno Q.

use anyhow::{Context, Result};
use std::process::Command;

const BRIDGE_APP_NAME: &str = "freeclaw-uno-q-bridge";
...
pub fn setup_uno_q_bridge(host: Option<&str>) -> Result<()> {
    ...
    if let Some(h) = host {
        if bridge_dir.exists() {
            ...
            anyhow::bail!(
                "Bridge app not found at {}. Run from freeclaw repo root.",
                bridge_dir.display()
            );
        }
    } else {
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    let status = Command::new("ssh")
        .args([
            ...
            "app",
            "start",
            "~/ArduinoApps/freeclaw-uno-q-bridge",
        ])
        .status()
    ...
    println!("FreeClaw Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("FreeClaw Bridge app started.");
    Ok(())
}

fn write_embedded_bridge(dest: &std::path::Path) -> Result<()> {
    let app_yaml = include_str!("../../firmware/freeclaw-uno-q-bridge/app.yaml");
    let sketch_ino = include_str!("../../firmware/freeclaw-uno-q-bridge/sketch/sketch.ino");
    let sketch_yaml = include_str!("../../firmware/freeclaw-uno-q-bridge/sketch/sketch.yaml");
    let main_py = include_str!("../../firmware/freeclaw-uno-q-bridge/python/main.py");
    let requirements = include_str!("../../firmware/freeclaw-uno-q-bridge/python/requirements.txt");
