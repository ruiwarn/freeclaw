...
const SERVICE_LABEL: &str = "com.freeclaw.daemon";
const WINDOWS_TASK_NAME: &str = "FreeClaw Daemon";
const MACOS_LAUNCHD_PATH: &str = "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin";
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "freeclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["freeclaw", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "freeclaw.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["freeclaw", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "freeclaw.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["freeclaw", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["freeclaw", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/freeclaw");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/freeclaw");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "freeclaw", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove freeclaw from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/freeclaw)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
"#,
        ...
        path = xml_escape(MACOS_LAUNCHD_PATH),
    ...
    println!("   Start with: freeclaw service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=FreeClaw daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "freeclaw.service"]));
...
/// Check if the freeclaw user exists and has expected properties.
...
fn check_freeclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "freeclaw"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser freeclaw && delgroup freeclaw",
            "addgroup -S freeclaw && adduser -S -s /sbin/nologin -H -D -G freeclaw freeclaw",
        )
    } else {
        ("userdel freeclaw", "useradd -r -s /sbin/nologin freeclaw")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'freeclaw' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'freeclaw' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/freeclaw" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: freeclaw user has home directory '{}' (expected /var/lib/freeclaw or /nonexistent)",
                        home
                    );
                }
...
fn ensure_freeclaw_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "freeclaw"]).output();
    if let Ok(output) = output {
        if output.status.success() {
            return check_freeclaw_user();
        }
    }
    ...
    if is_alpine {
        let group_output = Command::new("getent").args(["group", "freeclaw"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "freeclaw"])
                .output()
                .context("Failed to create freeclaw group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create freeclaw group: {}", stderr.trim());
            }
            println!("✅ Created system group: freeclaw");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create freeclaw user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create freeclaw user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "freeclaw"])
            .output()
            .context("Failed to create freeclaw user")?;
    ...
    println!("✅ Created system user: freeclaw");
    Ok(())
}

/// Change ownership of a path to freeclaw:freeclaw
#[cfg(unix)]
fn chown_to_freeclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["freeclaw:freeclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to freeclaw:freeclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_freeclaw(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_freeclaw(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "freeclaw:freeclaw", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to freeclaw:freeclaw: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_freeclaw(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".freeclaw"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'freeclaw' cannot write {} ({details}). \
             Re-run `sudo freeclaw service install` and ensure ownership is freeclaw:freeclaw.",
            path.display(),
        );
    }
    ...
    if path_str.contains("/home/") || path_str.contains(".cargo/bin") {
        eprintln!(
            "⚠️  Warning: Binary path '{}' appears to be in a user home directory.\n\
             ...
             sudo cp '{}' /usr/local/bin/freeclaw",
            exe_path.display(),
            exe_path.display()
        );
    }
...
command_user="freeclaw:freeclaw"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/freeclaw/access.log"
error_log="/var/log/freeclaw/error.log"
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/freeclaw");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo freeclaw service install"
        );
    }

    ensure_freeclaw_user()?;

    let exe = resolve_openrc_executable()?;
    warn_if_binary_in_home(&exe);

    let config_dir = Path::new("/etc/freeclaw");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/freeclaw");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_freeclaw(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by freeclaw:freeclaw)",
    ...
    chown_recursive_to_freeclaw(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/freeclaw");
    ...
    run_checked(Command::new("rc-update").args(["add", "freeclaw", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/freeclaw");
    println!("   Config path: /etc/freeclaw/config.toml");
    println!("   Start with: sudo freeclaw service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("freeclaw-daemon.cmd");
...
"#;
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/freeclaw.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "FreeClaw Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/freeclaw");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/freeclaw"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"freeclaw\""));
        assert!(script.contains("description=\"FreeClaw daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/freeclaw\""));
        assert!(script.contains("command_args=\"--config-dir /etc/freeclaw daemon\""));
        assert!(!script.contains("env FREECLAW_CONFIG_DIR"));
        assert!(!script.contains("env FREECLAW_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"freeclaw:freeclaw\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/freeclaw/access.log\""));
        assert!(script.contains("error_log=\"/var/log/freeclaw/error.log\""));
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/freeclaw");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/freeclaw");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/freeclaw");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/freeclaw"), true);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/freeclaw/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/freeclaw/workspace'".to_string(),
                "freeclaw".to_string()
            ]
        );
    }
