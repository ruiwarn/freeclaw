...
//! The default FreeClaw binary excludes it to maintain the 4.6 MB size target.
...
impl RuntimeAdapter for WasmRuntime {
    ...
    fn storage_path(&self) -> PathBuf {
        self.workspace_dir
            .as_ref()
            .map_or_else(|| PathBuf::from(".freeclaw"), |w| w.join(".freeclaw"))
    }
...
mod tests {
    ...
    fn wasm_storage_path_default() {
        let rt = WasmRuntime::new(default_config());
        assert!(rt.storage_path().to_string_lossy().contains("freeclaw"));
    }

    #[test]
    fn wasm_storage_path_with_workspace() {
        let rt = WasmRuntime::with_workspace(default_config(), PathBuf::from("/home/user/project"));
        assert_eq!(rt.storage_path(), PathBuf::from("/home/user/project/.freeclaw"));
    }
