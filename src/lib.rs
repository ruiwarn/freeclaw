...
Examples:
  ...
  freeclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `FreeClaw` workspace
    Openclaw {
...
Examples:
  ...
  freeclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  freeclaw cron add-every 60000 'Ping heartbeat'     # every minute
  freeclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  freeclaw cron once 2h 'Follow up on deployment'
  freeclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  freeclaw cron update <task-id> --expression '0 8 * * *'
  freeclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  freeclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  freeclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  freeclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash FreeClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash FreeClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  freeclaw peripheral flash
  freeclaw peripheral flash --port /dev/cu.usbmodem12345
  freeclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash FreeClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
