...
impl LucidMemory {
    ...
    pub fn new(workspace_dir: &Path, local: SqliteMemory) -> Self {
        let lucid_cmd = std::env::var("FREECLAW_LUCID_CMD")
            .unwrap_or_else(|_| Self::DEFAULT_LUCID_CMD.to_string());

        let token_budget = std::env::var("FREECLAW_LUCID_BUDGET")
            .ok()
