...
// in `~/.freeclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.freeclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(freeclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: freeclaw_dir.join(".secret_key"),
            enabled,
        }
    }
