...
const OPEN_SKILLS_SYNC_MARKER: &str = ".freeclaw-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.freeclaw/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("FREECLAW_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("FREECLAW_OPEN_SKILLS_DIR").ok();
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.freeclaw/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.freeclaw/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: freeclaw skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.freeclaw");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.freeclaw/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("FREECLAW_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("FREECLAW_OPEN_SKILLS_DIR");
