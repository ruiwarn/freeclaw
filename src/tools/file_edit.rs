...
mod tests {
    ...
    async fn file_edit_replaces_single_match() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_single");
    ...
    async fn file_edit_not_found() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_notfound");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello world")
            .await
    ...
    async fn file_edit_multiple_matches() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_multi");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_delete_via_empty_new_string() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_delete");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_rejects_empty_old_string() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_empty_old_string");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("freeclaw_test_file_edit_symlink_escape");
    ...
    async fn file_edit_blocks_symlink_target_file() {
        ...
        let root = std::env::temp_dir().join("freeclaw_test_file_edit_symlink_target");
        let workspace = root.join("workspace");
        let outside = root.join("outside");
    ...
    async fn file_edit_blocks_readonly_mode() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello")
            .await
    ...
    async fn file_edit_blocks_when_rate_limited() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_rate_limited");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello")
            .await
    ...
    async fn file_edit_nonexistent_file() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_nofile");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_edit_blocks_null_byte_in_path() {
        let dir = std::env::temp_dir().join("freeclaw_test_file_edit_null_byte");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
