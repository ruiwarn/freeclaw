    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|freeclaw|services")
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage FreeClaw proxy settings (scope: environment | freeclaw | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | freeclaw | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
