...
Python companion package for [FreeClaw](https://github.com/ruiwarn/freeclaw) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install freeclaw-tools[discord]
```
...
# Run the CLI
freeclaw-tools "List files in the current directory"
...
```python
from freeclaw_tools import tool
...
| Feature | Rust FreeClaw | freeclaw-tools |
|---------|---------------|----------------|
...
Use **Rust FreeClaw** for production edge deployments. Use **freeclaw-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
