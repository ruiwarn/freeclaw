...
impl ListenTool {
    pub fn new(config: RobotConfig) -> Self {
        let recordings_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".freeclaw/recordings"))
            .unwrap_or_else(|| PathBuf::from("/tmp/freeclaw_recordings"));
    ...
    async fn transcribe(&self, audio_path: &Path) -> Result<String> {
        ...
        // whisper.cpp model path (typically in ~/.freeclaw/models/)
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".freeclaw/models/ggml-{}.bin", model))
            })
            .unwrap_or_else(|| {
