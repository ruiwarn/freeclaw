...
impl SpeakTool {
    pub fn new(config: RobotConfig) -> Self {
        let audio_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".freeclaw/tts_cache"))
            .unwrap_or_else(|| PathBuf::from("/tmp/freeclaw_tts"));
    ...
    async fn speak(&self, text: &str, emotion: &str) -> Result<()> {
        ...
        let model_path = directories::UserDirs::new()
            .map(|d| {
                d.home_dir()
                    .join(format!(".freeclaw/models/piper/{}.onnx", voice))
            })
    ...
    async fn play_sound(&self, sound: &str) -> Result<()> {
        let sounds_dir = directories::UserDirs::new()
            .map(|d| d.home_dir().join(".freeclaw/sounds"))
            .unwrap_or_else(|| PathBuf::from("/usr/local/share/freeclaw/sounds"));
