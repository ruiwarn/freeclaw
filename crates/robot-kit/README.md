...
A complete toolkit for building AI-powered robots with FreeClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build FreeClaw with robot tools

```bash
# Clone and build
git clone https://github.com/ruiwarn/freeclaw
cd freeclaw
cargo build -p freeclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.freeclaw
cp crates/robot-kit/robot.toml ~/.freeclaw/
cp crates/robot-kit/SOUL.md ~/.freeclaw/workspace/
...
# Test in mock mode
./target/release/freeclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/freeclaw agent -m "Move forward 1 meter"
```
...
```rust
use freeclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `freeclaw`, add a thin adapter that maps this
...
mkdir freeclaw-robot-kit
cp -r target/release/freeclaw freeclaw-robot-kit/
cp -r examples/robot_kit freeclaw-robot-kit/
cp -r ~/.freeclaw freeclaw-robot-kit/dot-freeclaw

# Include models
mkdir -p freeclaw-robot-kit/models
cp ~/.freeclaw/models/ggml-base.bin freeclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf freeclaw-robot-kit.tar.gz freeclaw-robot-kit/

# Copy to USB
cp freeclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
