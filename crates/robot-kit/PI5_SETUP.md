...
Complete guide to setting up a FreeClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.freeclaw/models
cp models/ggml-base.bin ~/.freeclaw/models/
```
...
mkdir -p ~/.freeclaw/models/piper
cd ~/.freeclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.freeclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build FreeClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/freeclaw
cd freeclaw

# Build robot kit
cargo build --release -p freeclaw-robot-kit

# Build main freeclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.freeclaw
nano ~/.freeclaw/robot.toml
```

```toml
# ~/.freeclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.freeclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/freeclaw_sensors.fifo"
...
### Start FreeClaw Agent

```bash
# Configure FreeClaw to use robot tools
cat > ~/.freeclaw/config.toml << 'EOF'
...
cp ~/freeclaw/crates/robot-kit/SOUL.md ~/.freeclaw/workspace/

# Start agent
./target/release/freeclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/freeclaw_sensors.fifo ]; then
    mkfifo /tmp/freeclaw_sensors.fifo
fi
...
# Start freeclaw
cd ~/freeclaw
./target/release/freeclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/freeclaw-robot.service
sudo tee /etc/systemd/system/freeclaw-robot.service << 'EOF'
[Unit]
Description=FreeClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/freeclaw
...
sudo systemctl enable freeclaw-robot
sudo systemctl start freeclaw-robot

# Check status
sudo systemctl status freeclaw-robot
journalctl -u freeclaw-robot -f  # View logs
```
