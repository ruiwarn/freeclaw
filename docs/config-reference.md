# FreeClaw Config Reference (Operator-Oriented)
...
1. `FREECLAW_WORKSPACE` override (if set)
2. persisted `~/.freeclaw/active_workspace.toml` marker (if present)
3. default `~/.freeclaw/config.toml`

FreeClaw logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `freeclaw config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `freeclaw` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `freeclaw doctor traces --limit 20`
  - `freeclaw doctor traces --event tool_call_result --contains \"error\"`
  - `freeclaw doctor traces --id <trace-id>`
...
1. `FREECLAW_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `FREECLAW_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, FreeClaw generates one and prints an enrollment URI once.
...
| `state_file` | `~/.freeclaw/estop-state.json` | Persistent estop state path |
...
- Use CLI command `freeclaw estop` to engage and `freeclaw estop resume` to clear levels.
...
- Security-first default: FreeClaw does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `FREECLAW_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `FREECLAW_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `FREECLAW_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `FREECLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `freeclaw skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- FreeClaw requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
3. Validate with `freeclaw doctor` before restart/rollout.
...
- `FREECLAW_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `FREECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
