# Thiết kế Hardware Peripherals — FreeClaw

FreeClaw cho phép các vi điều khiển (MCU) và máy tính nhúng (SBC) **phân tích lệnh ngôn ngữ tự nhiên theo thời gian thực**, tổng hợp code phù hợp với từng phần cứng, và thực thi tương tác với ngoại vi trực tiếp.

## 1. Tầm nhìn

**Mục tiêu:** FreeClaw đóng vai trò là AI agent có hiểu biết về phần cứng, cụ thể:
...
**Hình dung trực quan:** FreeClaw = bộ não hiểu phần cứng. Ngoại vi = tay chân mà nó điều khiển.
...
FreeClaw chạy **trực tiếp trên thiết bị**. Board khởi động server gRPC/nanoRPC và giao tiếp với ngoại vi ngay tại chỗ.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  FreeClaw on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. FreeClaw truy xuất tài liệu theo board (ví dụ: bản đồ GPIO của ESP32)
...
FreeClaw chạy trên **máy chủ** và duy trì kết nối phần cứng tới thiết bị mục tiêu. Dùng cho phát triển, kiểm tra nội tâm, và nạp firmware.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  FreeClaw on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. FreeClaw nhận diện phần cứng đang kết nối (VID/PID, kiến trúc)
...
2. FreeClaw ghi/nạp firmware qua OpenOCD hoặc probe-rs
3. Xác nhận thành công

**Hoặc:**
1. FreeClaw tự phát hiện: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|-----------|-------------|---------------|
| FreeClaw chạy trên | Thiết bị (ESP32, RPi) | Máy chủ (Mac, Linux) |
...
Máy chủ chạy FreeClaw; ngoại vi chạy firmware tối giản. JSON đơn giản qua serial.

### Chế độ B: RPi làm Host (Native GPIO)

FreeClaw trên Pi; GPIO qua rppal hoặc sysfs. Không cần firmware riêng.
...
board = "esp32"
transport = "wifi"
# Edge-Native: FreeClaw runs on ESP32
```
...
1. **Khởi động:** FreeClaw nạp config, đọc `peripherals.boards`.
...
Dành cho RPC có kiểu dữ liệu, độ trễ thấp giữa FreeClaw và các peripheral:
...
- **freeclaw-firmware** hoặc **freeclaw-peripheral** — một crate/workspace riêng biệt.
...
- Người dùng nạp lên board; FreeClaw kết nối và tự phát hiện khả năng.

## 9. Các giai đoạn triển khai

### Phase 1: Skeleton ✅ (Hoàn thành)

- [x] Thêm trait `Peripheral`, config schema, CLI (`freeclaw peripheral list/add`)
- [x] Thêm flag `--peripheral` cho agent
- [x] Ghi tài liệu vào AGENTS.md

### Phase 2: Host-Mediated — Phát hiện phần cứng ✅ (Hoàn thành)

- [x] `freeclaw hardware discover`: liệt kê thiết bị USB (VID/PID)
- [x] Board registry: ánh xạ VID/PID → kiến trúc, tên (ví dụ: Nucleo-F401RE)
- [x] `freeclaw hardware introspect <path>`: memory map, danh sách peripheral
...
- [x] FreeClaw trên Raspberry Pi (native GPIO qua rppal)
...
- [x] Crate firmware `freeclaw-esp32` (`firmware/freeclaw-esp32`) — GPIO qua UART
- [x] ESP32 trong hardware registry (CH340 VID/PID)
- [ ] FreeClaw *chạy trực tiếp trên* ESP32 (WiFi + LLM, edge-native) — tương lai
- [ ] Thực thi Wasm hoặc dựa trên template cho logic do LLM tạo ra

**Cách dùng:** Nạp `firmware/freeclaw-esp32` vào ESP32, thêm `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` vào config.
...
- Chạy FreeClaw đầy đủ *trực tiếp trên* STM32 bare-metal (không có WiFi, RAM hạn chế) — dùng Host-Mediated thay thế
...
> *"Các board như ESP, Raspberry Pi, hoặc các board có WiFi có thể kết nối với LLM (Gemini hoặc mã nguồn mở). FreeClaw chạy trên thiết bị, tạo gRPC riêng, khởi động nó, và giao tiếp với ngoại vi. Người dùng hỏi qua WhatsApp: 'di chuyển cánh tay X' hoặc 'bật LED'. FreeClaw lấy tài liệu chính xác, viết code, thực thi, lưu trữ tối ưu, chạy, và bật LED — tất cả trên board phát triển.*
>
> *Với STM Nucleo kết nối qua USB/J-Link/Aardvark vào Mac: FreeClaw từ Mac truy cập phần cứng, cài đặt hoặc ghi những gì cần thiết lên thiết bị, và trả kết quả. Ví dụ: 'Hey FreeClaw, những địa chỉ khả dụng/đọc được trên thiết bị USB này là gì?' Nó có thể tự tìm ra thiết bị nào đang kết nối ở đâu và đưa ra gợi ý."*
