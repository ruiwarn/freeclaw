# Tài liệu tham khảo Providers — FreeClaw
...
3. Biến môi trường dự phòng chung: `FREECLAW_API_KEY`, sau đó là `API_KEY`
...
- Sau khi chuẩn hóa multimodal, FreeClaw gửi payload hình ảnh qua trường `messages[].images` gốc của Ollama.
- Nếu chọn provider không hỗ trợ vision, FreeClaw trả về lỗi rõ ràng thay vì âm thầm bỏ qua hình ảnh.
...
- Khám phá model: `freeclaw models refresh --provider nvidia`
