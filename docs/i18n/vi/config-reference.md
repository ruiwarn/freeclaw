...
1. Biến `FREECLAW_WORKSPACE` (nếu được đặt)
2. Marker `~/.freeclaw/active_workspace.toml` (nếu có)
3. Mặc định `~/.freeclaw/config.toml`

FreeClaw ghi log đường dẫn config đã giải quyết khi khởi động ở mức `INFO`:

- `Config loaded` với các trường: `path`, `workspace`, `source`, `initialized`

Lệnh xuất schema:

- `freeclaw config schema` (xuất JSON Schema draft 2020-12 ra stdout)
...
| `otel_service_name` | `freeclaw` | Tên dịch vụ gửi đến OTLP collector |
...
1. `FREECLAW_PROVIDER` (ghi đè tường minh, luôn thắng khi có giá trị)
...
- Dùng `FREECLAW_PROVIDER` khi cố ý muốn biến môi trường ghi đè provider đã cấu hình.
...
- Mặc định an toàn: FreeClaw **không** clone hay đồng bộ `open-skills` trừ khi `open_skills_enabled = true`.
- Ghi đè qua biến môi trường:
  - `FREECLAW_OPEN_SKILLS_ENABLED` chấp nhận `1/0`, `true/false`, `yes/no`, `on/off`.
  - `FREECLAW_OPEN_SKILLS_DIR` ghi đè đường dẫn kho khi có giá trị.
- Thứ tự ưu tiên: `FREECLAW_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` trong `config.toml` → mặc định `false`.
...
- FreeClaw yêu cầu Composio v3 tools với `toolkit_versions=latest` và thực thi với `version="latest"` để tránh bản tool mặc định cũ.
...
3. Kiểm tra bằng `freeclaw doctor` trước khi khởi động lại/triển khai.
