...
FreeClaw hỗ trợ tích hợp native với Mattermost thông qua REST API v4. Tích hợp này lý tưởng cho các môi trường self-hosted, riêng tư hoặc air-gapped nơi giao tiếp nội bộ là yêu cầu bắt buộc.
...
2. **Tài khoản Bot**:
    ...
    - Đặt username (ví dụ: `freeclaw-bot`).
...
| `mention_only` | (Tùy chọn) Khi `true`, chỉ các tin nhắn đề cập rõ ràng username bot (ví dụ `@freeclaw-bot`) mới được xử lý. Mặc định: `false`. |

## Cuộc hội thoại dạng Thread

FreeClaw hỗ trợ Mattermost thread ở cả hai chế độ:
- Nếu người dùng gửi tin nhắn trong một thread hiện có, FreeClaw luôn phản hồi trong cùng thread đó.
...
Khi `mention_only = true`, FreeClaw áp dụng bộ lọc bổ sung sau khi xác thực `allowed_users`:
