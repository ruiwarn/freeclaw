...
[INFO] ✓ Audit logging enabled (~/.config/freeclaw/audit.log)
...
$ freeclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/freeclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ freeclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ freeclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ freeclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ freeclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `freeclaw security --status`
