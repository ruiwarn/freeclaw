# FreeClaw Security Improvement Roadmap
...
FreeClaw already has **excellent application-layer security**:
...
## Comparison: FreeClaw vs PicoClaw vs Production Grade

| Feature | PicoClaw | FreeClaw Now | FreeClaw + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`freeclaw audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/freeclaw/audit.log"
sign_events = true
max_size_mb = 100
...
freeclaw security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
freeclaw audit --user @alice --since 24h
freeclaw audit --risk high --violations-only
freeclaw audit --verify-signatures

# Sandbox test
freeclaw sandbox --test
# → Testing isolation...
...
**FreeClaw is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, FreeClaw becomes:
...
**Value**: Transforms FreeClaw from "safe for testing" to "safe for production"
