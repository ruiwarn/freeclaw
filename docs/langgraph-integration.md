...
This guide explains how to use the `freeclaw-tools` Python package for consistent tool calling with any OpenAI-compatible LLM provider.

## Background

Some LLM providers, particularly Chinese models like GLM-5 (Zhipu AI), have inconsistent tool calling behavior when using text-based tool invocation. FreeClaw's Rust core uses structured tool calling via the OpenAI API format, but some models respond better to a different approach.
...
## Comparison with Rust FreeClaw

| Aspect | Rust FreeClaw | freeclaw-tools |
|--------|---------------|-----------------|
