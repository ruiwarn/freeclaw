# Network Deployment — FreeClaw on Raspberry Pi and Local Network

This document covers deploying FreeClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | FreeClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | FreeClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — FreeClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. FreeClaw on Raspberry Pi
...
```bash
freeclaw daemon --host 127.0.0.1 --port 42617
```

- Gateway binds to `127.0.0.1` — not reachable from other machines
- Telegram channel works: FreeClaw polls Telegram API (outbound)
...
```bash
freeclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `freeclaw tunnel` (see tunnel docs).

3. FreeClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- FreeClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `freeclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `freeclaw daemon` service).
...
- [ ] Run `freeclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
FreeClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `freeclaw` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/freeclaw`
- Config directory: `/etc/freeclaw/`
- Log directory: `/var/log/freeclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo freeclaw service install` automatically prepares `/etc/freeclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `freeclaw` service user.

If no prior runtime state is available to migrate, create `/etc/freeclaw/config.toml` before starting the service.
...
# Check status
sudo rc-service freeclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service freeclaw start` | Start the daemon |
| `sudo rc-service freeclaw stop` | Stop the daemon |
| `sudo rc-service freeclaw status` | Check service status |
| `sudo rc-service freeclaw restart` | Restart the daemon |
| `sudo freeclaw service status` | FreeClaw status wrapper (uses `/etc/freeclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/freeclaw/access.log` |
| Errors/stderr | `/var/log/freeclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/freeclaw/error.log
```
...
sudo rc-service freeclaw stop
sudo rc-update del freeclaw default

# Remove init script
sudo freeclaw service uninstall
```
...
- The service runs as the `freeclaw:freeclaw` user (least privilege)
- Config must be at `/etc/freeclaw/config.toml` (explicit path in init script)
- If the `freeclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo freeclaw service install`
- [ ] Enable: `sudo rc-update add freeclaw default`
- [ ] Start: `sudo rc-service freeclaw start`
- [ ] Verify: `sudo rc-service freeclaw status`
- [ ] Check logs: `/var/log/freeclaw/error.log`
