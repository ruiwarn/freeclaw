...
- CLI `freeclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.freeclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.freeclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   freeclaw sop list
   freeclaw sop validate
   freeclaw sop show deploy-prod
   ```
