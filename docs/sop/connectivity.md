...
FreeClaw routes MQTT/webhook/cron/peripheral events through a unified SOP dispatcher (`dispatch_sop_event`).
...
| **Cron not firing** | daemon not running or invalid expression | run `freeclaw daemon`; check logs for cron parse warnings |
