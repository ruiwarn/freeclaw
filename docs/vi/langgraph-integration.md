...
Hướng dẫn này giải thích cách sử dụng gói Python `freeclaw-tools` để gọi tool nhất quán với bất kỳ LLM provider nào tương thích OpenAI.

## Bối cảnh

Một số LLM provider, đặc biệt là các model Trung Quốc như GLM-5 (Zhipu AI), có hành vi gọi tool không nhất quán khi dùng phương thức text-based tool invocation. Core Rust của FreeClaw sử dụng structured tool calling theo định dạng OpenAI API, nhưng một số model phản hồi tốt hơn với cách tiếp cận khác.
...
```python
import asyncio
from freeclaw_tools import create_agent, shell, file_read, file_write
...
```python
from freeclaw_tools import tool, create_agent
...
```python
import os
from freeclaw_tools.integrations import DiscordBot
...
# Single message
freeclaw-tools "What is the current date?"

# Interactive mode
freeclaw-tools -i
```

## So sánh với Rust FreeClaw

| Khía cạnh | Rust FreeClaw | freeclaw-tools |
|--------|---------------|-----------------|
