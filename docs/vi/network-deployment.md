# Triển khai mạng — FreeClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai FreeClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | FreeClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | FreeClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — FreeClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. FreeClaw trên Raspberry Pi
...
Chỉnh sửa `~/.freeclaw/config.toml`:
...
- Channel Telegram hoạt động: FreeClaw poll Telegram API (outbound)
...
```bash
freeclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `freeclaw tunnel` (xem tài liệu tunnel).

3. FreeClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- FreeClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `freeclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `freeclaw daemon`).
...
- [ ] Chạy `freeclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
