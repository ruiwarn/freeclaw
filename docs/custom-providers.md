...
FreeClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export FREECLAW_API_KEY="your-api-key"
freeclaw agent
```

## llama.cpp Server (Recommended Local Setup)

FreeClaw includes a first-class local provider for `llama-server`:
...
```bash
freeclaw models refresh --provider llamacpp
freeclaw agent -m "hello"
```

You do not need to export `FREECLAW_API_KEY=dummy` for this flow.

## SGLang Server

FreeClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
freeclaw models refresh --provider sglang
freeclaw agent -m "hello"
```
...
FreeClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
freeclaw models refresh --provider vllm
freeclaw agent -m "hello"
```
