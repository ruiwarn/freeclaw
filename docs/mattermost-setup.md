...
FreeClaw supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
2.  **Bot Account**:
    ...
    - Set a username (e.g., `freeclaw-bot`).
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@freeclaw-bot`) are processed. Default: `false`. |

## Threaded Conversations

FreeClaw supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, FreeClaw always replies within that same thread.
...
When `mention_only = true`, FreeClaw applies an extra filter after `allowed_users` authorization:
