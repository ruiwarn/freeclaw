...
3. Generic fallback env vars: `FREECLAW_API_KEY` then `API_KEY`
...
- After multimodal normalization, FreeClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, FreeClaw returns a structured capability error instead of silently ignoring images.
...
- FreeClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `freeclaw models refresh --provider llamacpp`
...
- Model discovery: `freeclaw models refresh --provider sglang`
...
- Model discovery: `freeclaw models refresh --provider vllm`
...
- Model discovery: `freeclaw models refresh --provider osaurus`
...
- Model discovery: `freeclaw models refresh --provider nvidia`
