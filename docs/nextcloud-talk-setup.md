...
This guide covers native Nextcloud Talk integration for FreeClaw.
...
Add this section in `~/.freeclaw/config.toml`:
...
freeclaw daemon
# or
freeclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, FreeClaw verifies:
...
- FreeClaw ignores bot-originated webhook events (`actorType = bots`).
- FreeClaw ignores non-message/system events.
...
3. Confirm FreeClaw receives and replies in the same room.
