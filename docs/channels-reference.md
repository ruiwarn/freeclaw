...
This document is the canonical reference for channel configuration in FreeClaw.
...
6. **Stale runtime state**: config changed but `freeclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.freeclaw/config.toml`.
...
FreeClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `freeclaw channel list`, `freeclaw channel doctor`, and `freeclaw channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@freeclaw:matrix.example.com"   # optional, recommended for E2EE
...
port = 6697
nickname = "freeclaw-bot"
username = "freeclaw"              # optional
channels = ["#freeclaw"]
allowed_users = ["*"]
...
- `FREECLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `FREECLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `freeclaw daemon` after config changes.
...
```bash
RUST_LOG=info freeclaw daemon 2>&1 | tee /tmp/freeclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/freeclaw.log
```
