# FreeClaw on Nucleo-F401RE — Step-by-Step Guide

Run FreeClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

FreeClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
FreeClaw includes everything for Nucleo-F401RE:
...
| Flash command | `freeclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via FreeClaw

From the freeclaw repo root:

```bash
freeclaw peripheral flash-nucleo
```

This builds `firmware/freeclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `freeclaw daemon` or `freeclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `freeclaw peripheral list` to verify.
