# Adding Boards and Tools — FreeClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to FreeClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.freeclaw/config.toml)
freeclaw peripheral add nucleo-f401re /dev/ttyACM0
freeclaw peripheral add arduino-uno /dev/cu.usbmodem12345
freeclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, FreeClaw can index PDF files:
...
2. **Add to config** — `freeclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `freeclaw peripheral list` | List configured boards |
| `freeclaw peripheral add <board> <path>` | Add board (writes config) |
| `freeclaw peripheral flash` | Flash Arduino firmware |
| `freeclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `freeclaw hardware discover` | List USB devices |
| `freeclaw hardware info` | Chip info via probe-rs |
