# Hardware Peripherals Design — FreeClaw

FreeClaw enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** FreeClaw acts as a hardware-aware AI agent that:
...
**Mental model:** FreeClaw = brain that understands hardware. Peripherals = arms and legs it controls.
...
FreeClaw runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.
...
2. FreeClaw fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
FreeClaw runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.
...
2. FreeClaw identifies connected hardware (VID/PID, architecture)
...
2. FreeClaw writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. FreeClaw auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| FreeClaw runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs FreeClaw; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

FreeClaw on Pi; GPIO via rppal or sysfs. No separate firmware.
...
1. **Startup:** FreeClaw loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between FreeClaw and peripherals:
...
- **freeclaw-firmware** or **freeclaw-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; FreeClaw connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`freeclaw peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `freeclaw hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `freeclaw hardware introspect <path>`: memory map, peripheral list
...
- [x] FreeClaw on Raspberry Pi (native GPIO via rppal)
...
- [x] `freeclaw-esp32` firmware crate (`firmware/freeclaw-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] FreeClaw *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/freeclaw-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full FreeClaw *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). FreeClaw runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. FreeClaw gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: FreeClaw from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey FreeClaw, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
