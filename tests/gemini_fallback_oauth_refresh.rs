...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.freeclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.freeclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let freeclaw_dir = PathBuf::from(home).join(".freeclaw");
    let auth_profiles_path = freeclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: freeclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: freeclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.freeclaw/auth-profiles.json (with expired token)
    let provider = freeclaw::providers::create_provider("gemini", None)?;
