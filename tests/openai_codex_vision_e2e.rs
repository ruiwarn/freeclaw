...
use anyhow::Result;
use freeclaw::providers::{ChatMessage, ChatRequest, Provider, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = freeclaw::providers::create_provider_with_options("openai-codex", None, &opts)?;
