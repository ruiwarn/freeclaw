//! Performance benchmarks for FreeClaw hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/ruiwarn/freeclaw/issues/618 (item 7)
...
use freeclaw::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use freeclaw::config::MemoryConfig;
use freeclaw::memory;
use freeclaw::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "freeclaw architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "freeclaw"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about freeclaw agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("freeclaw agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
