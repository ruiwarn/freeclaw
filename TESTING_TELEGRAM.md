...
This guide covers testing the Telegram channel integration for FreeClaw.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug freeclaw channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: freeclaw onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time freeclaw channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep freeclaw`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/freeclaw` |
...
```bash
# Maximum logging
RUST_LOG=trace freeclaw channel start
...
# Validate config
cat ~/.freeclaw/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [FreeClaw Main README](README.md)
- [Contributing Guide](CONTRIBUTING.md)
- [Issue Tracker](https://github.com/theonlyhennygod/freeclaw/issues)
