...
<h1 align="center">FreeClaw 🦀（简体中文）</h1>
...
| 日期（UTC） | 级别 | 通知 | 处理建议 |
|---|---|---|---|
| 2026-02-19 | _紧急_ | 我们与 `openagen/freeclaw` 及 `freeclaw.org` **没有任何关系**。`freeclaw.org` 当前会指向 `openagen/freeclaw` 这个 fork，并且该域名/仓库正在冒充我们的官网与官方项目。 | 请不要相信上述来源发布的任何信息、二进制、募资活动或官方声明。请仅以[本仓库](https://github.com/ruiwarn/freeclaw)和已验证官方社媒为准。 |
| 2026-02-21 | _重要_ | 我们的官网现已上线：[freeclawlabs.ai](https://freeclawlabs.ai)。感谢大家一直以来的耐心等待。我们仍在持续发现冒充行为，请勿参与任何未经我们官方渠道发布、但打着 FreeClaw 名义进行的投资、募资或类似活动。 | 一切信息请以[本仓库](https://github.com/ruiwarn/freeclaw)为准；也可关注 [X（@freeclawlabs）](https://x.com/freeclawlabs?s=21)、[Telegram（@freeclawlabs）](https://t.me/freeclawlabs)、[Facebook（群组）](https://www.facebook.com/groups/freeclaw)、[Reddit（r/freeclawlabs）](https://www.reddit.com/r/freeclawlabs/) 与 [小红书账号](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) 获取官方最新动态。 |
...
FreeClaw 是一个高性能、低资源占用、可组合的自主智能体运行时。FreeClaw 是面向智能代理工作流的**运行时操作系统** — 它抽象了模型、工具、记忆和执行层，使代理可以一次构建、随处运行。
...
> 说明：FreeClaw 的数据来自 release 构建，并通过 `/usr/bin/time -l` 测得。OpenClaw 需要 Node.js 运行时环境，仅该运行时通常就会带来约 390MB 的额外内存占用；NanoBot 需要 Python 运行时环境。PicoClaw 与 FreeClaw 为静态二进制。

<p align="center">
  <img src="zero-claw.jpeg" alt="FreeClaw vs OpenClaw 对比图" width="800" />
</p>
...
- Release 二进制：`8.8M`
- `freeclaw --help`：约 `0.02s`，峰值内存约 `3.9MB`
- `freeclaw status`：约 `0.01s`，峰值内存约 `4.1MB`
...
FreeClaw 现已支持基于订阅的原生鉴权配置（多账号、静态加密存储）。

- 配置文件：`~/.freeclaw/auth-profiles.json`
- 加密密钥：`~/.freeclaw/.secret_key`
...
<p align="center">
  <img src="docs/architecture.svg" alt="FreeClaw 架构图" width="900" />
</p>

| 子系统 | Trait | 内置实现 | 扩展方式 |
|--------|-------|----------|----------|
| **AI 模型** | `Provider` | 通过 `freeclaw providers` 查看（当前 28 个内置 + 别名，以及自定义端点） | `custom:https://your-api.com`（OpenAI 兼容）或 `anthropic-custom:https://your-api.com` |
...
配置了不支持的 `runtime.kind` 时，FreeClaw 会以明确的错误退出，而非静默回退到 native。
