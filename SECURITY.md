...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/freeclaw/security/advisories/new)
...
FreeClaw implements defense-in-depth security:
...
FreeClaw Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' freeclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace freeclaw gateway
```
