...
<p align="center">
  FreeClaw est le <strong>système d'exploitation runtime</strong> pour les workflows agentiques — une infrastructure qui abstrait les modèles, outils, mémoire et exécution pour construire des agents une fois et les exécuter partout.
</p>
...
| ---------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critique_  | Nous ne sommes **pas affiliés** à `openagen/freeclaw` ou `freeclaw.org`. Le domaine `freeclaw.org` pointe actuellement vers le fork `openagen/freeclaw`, et ce domaine/dépôt usurpe l'identité de notre site web/projet officiel.                                                                                                                                                                                 | Ne faites pas confiance aux informations, binaires, levées de fonds ou annonces provenant de ces sources. Utilisez uniquement [ce dépôt](https://github.com/ruiwarn/freeclaw) et nos comptes sociaux vérifiés.                                                                                                                                                                                                                                                                                                                                                          |
| 2026-02-21 | _Important_ | Notre site officiel est désormais en ligne : [freeclawlabs.ai](https://freeclawlabs.ai). Merci pour votre patience pendant cette attente. Nous constatons toujours des tentatives d'usurpation : ne participez à aucune activité d'investissement/financement au nom de FreeClaw si elle n'est pas publiée via nos canaux officiels.                                                                                                                   | Utilisez [ce dépôt](https://github.com/ruiwarn/freeclaw) comme source unique de vérité. Suivez [X (@freeclawlabs)](https://x.com/freeclawlabs?s=21), [Telegram (@freeclawlabs)](https://t.me/freeclawlabs), [Facebook (groupe)](https://www.facebook.com/groups/freeclaw), [Reddit (r/freeclawlabs)](https://www.reddit.com/r/freeclawlabs/), et [Xiaohongshu](https://www.xiaohongshu.com/user/profile/67cbfc43000000000d008307?xsec_token=AB73VnYnGNx5y36EtnnZfGmAmS-6Wzv8WMuGpfwfkg6Yc%3D&xsec_source=pc_search) pour les mises à jour officielles. |
...
### Pourquoi les équipes choisissent FreeClaw
...
## Instantané de Benchmark (FreeClaw vs OpenClaw, Reproductible)
...
> Notes : Les résultats FreeClaw sont mesurés sur des builds de production utilisant `/usr/bin/time -l`. OpenClaw nécessite le runtime Node.js (typiquement ~390 Mo de surcharge mémoire supplémentaire), tandis que NanoBot nécessite le runtime Python. PicoClaw et FreeClaw sont des binaires statiques. Les chiffres RAM ci-dessus sont la mémoire runtime ; les exigences de compilation build-time sont plus élevées.

<p align="center">
  <img src="zero-claw.jpeg" alt="Comparaison FreeClaw vs OpenClaw" width="800" />
</p>
...
- `freeclaw --help` : environ `0.02s` de temps réel, ~`3.9 Mo` d'empreinte mémoire maximale
- `freeclaw status` : environ `0.01s` de temps réel, ~`4.1 Mo` d'empreinte mémoire maximale
...
Le script `bootstrap.sh` installe Rust, clone FreeClaw, le compile, et configure votre environnement de développement initial :

```bash
curl -fsSL https://raw.githubusercontent.com/ruiwarn/freeclaw/main/bootstrap.sh | bash
```

Ceci va :

1. Installer Rust (si absent)
2. Cloner le dépôt FreeClaw
3. Compiler FreeClaw en mode release
4. Installer `freeclaw` dans `~/.cargo/bin/`
5. Créer la structure de workspace par défaut dans `~/.freeclaw/workspace/`
6. Générer un fichier de configuration `~/.freeclaw/workspace/config.toml` de démarrage

Après le bootstrap, relancez votre shell ou exécutez `source ~/.cargo/env` pour utiliser la commande `freeclaw` globalement.
...
1. Configurez vos fournisseurs d'IA dans `~/.freeclaw/workspace/config.toml`
2. Consultez la [référence de configuration](docs/config-reference.md) pour les options avancées
3. Lancez l'agent : `freeclaw agent start`
4. Testez via votre canal préféré (voir [référence des canaux](docs/channels-reference.md))

## Configuration

Éditez `~/.freeclaw/workspace/config.toml` pour configurer les fournisseurs, canaux et comportement du système.
...
FreeClaw prend en charge deux backends d'exécution de code :
...
freeclaw init                # Initialise un nouveau workspace
freeclaw status              # Affiche l'état du daemon/agent
freeclaw config validate     # Vérifie la syntaxe et les valeurs de config.toml

# Gestion du daemon
freeclaw daemon start        # Démarre le daemon en arrière-plan
freeclaw daemon stop         # Arrête le daemon en cours d'exécution
freeclaw daemon restart      # Redémarre le daemon (rechargement de config)
freeclaw daemon logs         # Affiche les journaux du daemon

# Gestion de l'agent
freeclaw agent start         # Démarre l'agent (nécessite daemon en cours d'exécution)
freeclaw agent stop          # Arrête l'agent
freeclaw agent restart       # Redémarre l'agent (rechargement de config)

# Opérations de pairing
freeclaw pairing init        # Génère un nouveau secret de pairing
freeclaw pairing rotate      # Fait tourner le secret de pairing existant

# Tunneling (pour exposition publique)
freeclaw tunnel start        # Démarre un tunnel vers le daemon local
freeclaw tunnel stop         # Arrête le tunnel actif

# Diagnostic
freeclaw doctor              # Exécute les vérifications de santé du système
freeclaw version             # Affiche la version et les informations de build
```
...
enabled = true
homeserver_url = "https://matrix.org"
username = "@freeclaw:matrix.org"
password = "..."
device_name = "freeclaw-prod"
e2ee_enabled = true
```

Invitez `@freeclaw:matrix.org` dans une salle chiffrée, et le bot répondra avec le chiffrement complet. Voir [Guide Matrix E2EE](docs/matrix-e2ee-guide.md) pour la configuration de vérification de dispositif.
...
[memory]
kind = "sqlite"
path = "~/.freeclaw/workspace/memory/conversations.db"
...
[memory]
kind = "markdown"
path = "~/.freeclaw/workspace/memory/"
```
...
FreeClaw prend en charge les endpoints compatibles OpenAI :
...
FreeClaw fournit des outils intégrés pour l'exécution de code, l'accès au système de fichiers et la récupération web :
...
```bash
freeclaw daemon start
freeclaw agent start
```
...
sudo cp deployment/systemd/freeclaw-daemon.service /etc/systemd/system/
sudo cp deployment/systemd/freeclaw-agent.service /etc/systemd/system/

# Activez et démarrez les services
sudo systemctl enable freeclaw-daemon freeclaw-agent
sudo systemctl start freeclaw-daemon freeclaw-agent

# Vérifiez le statut
sudo systemctl status freeclaw-daemon
sudo systemctl status freeclaw-agent
```
...
```bash
# Compilez l'image
docker build -t freeclaw:latest .

# Exécutez le conteneur
docker run -d \
  --name freeclaw \
  -v ~/.freeclaw/workspace:/workspace \
...
FreeClaw est conçu pour fonctionner sur du matériel à faible consommation d'énergie :
...
Exposez votre daemon FreeClaw local au réseau public via des tunnels sécurisés :

```bash
freeclaw tunnel start --provider cloudflare
```
...
FreeClaw implémente plusieurs couches de sécurité :

### Pairing

Le daemon génère un secret de pairing au premier lancement stocké dans `~/.freeclaw/workspace/.pairing`. Les clients (agent, CLI) doivent présenter ce secret pour se connecter.

```bash
freeclaw pairing rotate  # Génère un nouveau secret et invalide l'ancien
```
...
FreeClaw journalise vers `~/.freeclaw/workspace/logs/` par défaut. Les journaux sont stockés par composant :

```
~/.freeclaw/workspace/logs/
...
Support de métriques Prometheus pour la surveillance en production à venir. Suivi dans [#234](https://github.com/ruiwarn/freeclaw/issues/234).

## Compétences (Skills)

FreeClaw prend en charge les compétences personnalisées — des modules réutilisables qui étendent les capacités du système.

### Définition de Compétence

Les compétences sont stockées dans `~/.freeclaw/workspace/skills/<nom-compétence>/` avec cette structure :
...
FreeClaw prend en charge les [Open Skills](https://github.com/openagents-com/open-skills) — un système modulaire et agnostique des fournisseurs pour étendre les capacités des agents IA.
...
Vous pouvez également surcharger au runtime avec `FREECLAW_OPEN_SKILLS_ENABLED` et `FREECLAW_OPEN_SKILLS_DIR`.
...
FreeClaw est configuré pour utiliser `rustls` pour les dépendances HTTP/TLS ; `--locked` maintient le graphe transitif déterministe sur les environnements vierges.
...
Si FreeClaw aide votre travail et que vous souhaitez soutenir le développement continu, vous pouvez faire un don ici :
...
**Ceci est le seul dépôt officiel FreeClaw :**

> <https://github.com/ruiwarn/freeclaw>

Tout autre dépôt, organisation, domaine ou package prétendant être "FreeClaw" ou impliquant une affiliation avec FreeClaw Labs est **non autorisé et non affilié à ce projet**. Les forks non autorisés connus seront listés dans [TRADEMARK.md](TRADEMARK.md).

Si vous rencontrez une usurpation d'identité ou une utilisation abusive de marque, veuillez [ouvrir une issue](https://github.com/ruiwarn/freeclaw/issues).

---

## Licence

FreeClaw est sous double licence pour une ouverture maximale et la protection des contributeurs :
...
Le nom **FreeClaw** et le logo sont des marques déposées de FreeClaw Labs. Cette licence n'accorde pas la permission de les utiliser pour impliquer une approbation ou une affiliation. Voir [TRADEMARK.md](TRADEMARK.md) pour les utilisations permises et interdites.
...
- Nouvelle `Skill` → `~/.freeclaw/workspace/skills/<n>/`

---

**FreeClaw** — Zéro surcharge. Zéro compromis. Déployez n'importe où. Échangez n'importe quoi. 🦀
...
<p align="center">
  <a href="https://www.star-history.com/#ruiwarn/freeclaw&type=date&legend=top-left">
    <picture>
     ...
     <img alt="Graphique Historique des Étoiles" src="https://api.star-history.com/svg?repos=ruiwarn/freeclaw&type=date&legend=top-left" />
    </picture>
  </a>
</p>
