#!/bin/bash
# FreeClaw Telegram Integration Test Suite
...
if [ -f "target/release/freeclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/freeclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/freeclaw 2>/dev/null || stat -c%s target/release/freeclaw)
...
CONFIG_PATH="$HOME/.freeclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'freeclaw onboard' first"
    fi
else
    warn "No config file found - run 'freeclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/freeclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
